...
+
NOTE: For performance, only national-level series (state_code = "00") are ingested (~800K series instead of ~6M). Data rows are filtered to match existing series_ids. This reduces data volume by ~80% while preserving all data needed for national-level analysis.
+
