...
* Series metadata (`oe.series`)
* Data values (`oe.data.0.Current`)
+
...
----
+
. `db:export` defaults to `meaningfulness_scores`, and supports `EXPORT_TABLE`, `EXPORT_PATH`, and `EXPORT_BATCH_SIZE`.
+
...
* `EXPORT_TABLE` - Table name to export (default `meaningfulness_scores`)
* `EXPORT_PATH` - Output path for CSV export
* `EXPORT_BATCH_SIZE` - Rows per export batch (default `5000`)
